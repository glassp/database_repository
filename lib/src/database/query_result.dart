...
class QueryResult {
    ...
    QueryResult._(this.query, {required this.wasSuccessful, this.payload});

    /// Creates a QueryResult for a failed Query
    factory QueryResult.failed(Query query, {String errorMsg = ""}) {
        return QueryResult._(query, {wasSuccessful: false, payload: const {'ERROR': errorMsg}});
    }

    /// Creates a QueryResult for a successful Query
    factory QueryResult.success(Query query, {JSON payload = const {}}) {
        return QueryResult._(query, {wasSuccessful: true, payload: payload});
    }
