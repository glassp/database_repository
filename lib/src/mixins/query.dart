...
mixin QueryMixin<T extends DatabaseStorable> {
    ...
    Query getReadQuery(String id, {required Type type, JSON where = const {}}) => Query(
            entityName: type.toString(), 
            action: QueryAction.read, 
            payload: {"id": id}
        );
    ...
    Query getReadAllQuery({required Type type}) => getReadAllWhereQuery(type: type);

    /// generates a query that execute a delete operation on the database
    Query getDeleteQuery(T element) => Query(entityName: element.runtimeType.toString(), action: QueryAction.delete, payload: element.toJson());
